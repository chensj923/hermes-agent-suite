...
| 路径 | 内容 |
| --- | --- |
| `%APPDATA%\@hermes\buddy-desktop` | **只清缓存**：Chromium/Electron 缓存（Cache / Code Cache / GPUCache / Local Storage / Session Storage / blob_storage / Network 等）、`logs\`、`server-deploy\`（下次启动按当前版本重新解压）。**保留** `buddy.connection`（连接配置）、`agents.json`（智能体与模型选择）、`memory\` |
| `%APPDATA%\hermesbuddy-desktop`、`%APPDATA%\Hermes Buddy` | 早期版本遗留目录，同样只清缓存 |
| `%LOCALAPPDATA%\@hermesbuddy-desktop-updater` 等 | electron-updater 下载缓存，单份 80MB 上下 |

卸载时则**整个删除** userData（连接配置一并清掉），并安装时一并清理以上目录。
...
2. 升级**不会**丢连接配置与智能体配置，无需重填连接信息；但若服务端仍是旧通道，Buddy 仍会提示重新部署（通道版本协商）。
...
- **多模态输入（v3.6.0）**：输入框左侧可添加图片 / 文件，也可以直接录音或选视频。
  图片和文本文件内容会作为多模态消息发给 AI；**语音和视频先在本机处理**——
  本地 Whisper 转写成文字、ffmpeg 按间隔抽关键帧，只把「文字 + 关键帧图片」发出去，
  **原始音视频不出本机**。详见第 9.1 节。
- **模型不被上游接受**（v3.4.5）：某些上游对模型有额外限制（典型是火山方舟 coding plan 只接受特定模型，
  选错会 404 `UnsupportedModel`）。此时服务端会自动改用默认模型继续，聊天区出现一条
  「模型 X 不被上游支持，已自动改用默认模型 Y」的提示，该模型也会从下拉里移除。
  想固定用别的模型，到「智能体 → 模型」改选即可。

### 9.1 本地媒体引擎（语音 / 视频转写）

语音视频的本地处理依赖两个可选组件，**不装也能正常用文字、图片和文件**，只是语音/视频无法转写：

| 组件 | 用途 | 体积 |
|---|---|---|
| Whisper（whisper-cli.exe） | 语音 → 文字 | 约 8 MB |
| 语音模型（ggml-*.bin） | 转写用模型，可选 tiny / base / small | 78 / 148 / 488 MB |
| ffmpeg（ffmpeg.exe） | 视频抽音轨 + 抽关键帧 | 约 111 MB |

**一键安装**：「设置 → 本地媒体引擎」里点「一键安装缺失组件」，或等发语音/视频时聊天区弹出的提示里点
「安装本地转写引擎」。安装过程会显示下载进度。

实现要点：

- 装在 `%APPDATA%\@hermes\buddy-desktop\media`，**不写 PATH、不写注册表、不需要管理员权限**。
- whisper.cpp 的 release tag 是滚动构建号（如 `b5130`），写死会 404，所以运行时先查 GitHub API
  找最新的 `whisper-bin-x64.zip`，API 不可用才回落兜底地址。
- 下载走多镜像顺序尝试：GitHub 直链优先试 `ghfast.top` 加速，模型走 `hf-mirror.com`，失败回落官方源。
- 也支持完全手动：把 `whisper-cli.exe` / `ffmpeg.exe` / `ggml-*.bin` 放进引擎目录即可
  （目录可用「打开引擎目录」按钮直达），或用 `BUDDY_WHISPER_BIN` / `BUDDY_FFMPEG_BIN` /
  `BUDDY_WHISPER_MODEL` 环境变量指定。
- 缺引擎只会降级（视频仍尽量抽关键帧），**绝不会让消息发不出去**。

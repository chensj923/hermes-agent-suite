...
class SessionManager {
  ...
  async connectChannel(normalized) {
    ...
    // Gateway 会话登记是可选能力，不通只降级（本机工具链路不依赖它）。
    let session = null;
    let health = null;
    let deployment = null;
    let gatewayWarning = null;
    ...
    if (this.provisioning && normalized.baseUrl) {
      ...
      if (this.session && normalized.managementUrl) {
        try {
          const managementGateway = this.provisioning.createGateway({ baseUrl: normalized.managementUrl, apiKey: normalized.apiKey });
          deployment = await this.provisioning.provision({
            gateway: managementGateway, product: this.product, deployment: this.deployment, registry: this.registry
          });
        } catch (error) {
          if (error && (error.code === 'not_found' || error.status === 404)) {
            this.logger.info('provisioning-not-available', { managementUrl: normalized.managementUrl, message: error.message });
      ...
      gatewayWarning = '未配置 Gateway，跳过会话登记。';
    }
    ...
    this.session = session;
    this.messages = [];
    this.logger.info('connected-channel', { channelUrl: normalized.channelUrl, gateway: health ? 'ok' : 'degraded' });
    return {
      connection: publicView(saved),
      session,
      deployment,
  ...
  async resumeChannel(stored) {
    ...
    this.session = session;
    this.messages = [];
    this.logger.info('resumed-channel', { channelUrl: stored.channelUrl, gateway: session ? 'ok' : 'degraded' });
    return { ok: true, connection: publicView(stored), workspace: this.describeWorkspace(),
             gatewayWarning: session ? null : 'Gateway 未连通，本机工具不受影响。' };
  }
  ...
  async send({ requestId, text, onConfirm, model }, onEvent) {
    ...
    if (this.channel) {
      ...
      try {
        ...
        const message = error.message || '通道错误';
        emit({ kind: 'error', type: 'error', message });
        const wrapped = new Error(message); wrapped.code = 'channel_error'; throw wrapped;
      } finally {
